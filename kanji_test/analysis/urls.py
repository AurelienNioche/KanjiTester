...
from kanji_test.analysis.views import basic, chart_dashboard, data, raters, pivots, rater_csv, pivots_by_syllabus, \
...
urlpatterns = ( #patterns('kanji_test.analysis.views',
    ...
    url(r'^data/(?P<name>[a-zA-Z0-9_]+)\.(?P<format>[a-z]+)$', data, name='analysis_data'),
    url(r'^raters/$', raters, name='analysis_raters'),
    url(r'^raters/(?P<rater_id>[0-9]+)/$', rater_detail, name='analysis_rater_detail'),
    url(r'^raters/rater_(?P<rater_id>[0-9]+)_(?P<data_type>word|kanji)\.csv', rater_csv, name='analysis_rater_csv'),

...
from user_model import plugin_api
...
class Question(models.Model):
    ...
    pivot_type = models.CharField(
        max_length=1, choices=PIVOT_TYPES,
...
class TestSet(models.Model):
    ...
    def from_user(user, n_questions=settings.QUESTIONS_PER_SET):
        ...
        test_set = TestSet(
            user=user, random_seed=random.randrange(0, 2**30),
            set_type=set_type)
        test_set.save()

        from .drill import load_plugins
        from .plugin_api import UnsupportedItem
        question_plugins = load_plugins(plugin_set)
        items = user.profile.syllabus.get_random_items(n_questions)
        questions = []
        for item in items:

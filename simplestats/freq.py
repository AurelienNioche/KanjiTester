...
class FreqDist(dict):
    ...
    def dump(self, filename):
        ...
        for key, count in sorted(self.items(), key=lambda x: x[1],  
                reverse=True):
            key = _escape_spaces(str(key))
            print("%s %d" % (key, count), file=o_stream)
        o_stream.close()
        return
    ...
    def merge(self, rhs_dist):
        for sample, count in rhs_dist.items():
...
class ConditionalFreqDist(dict):
    ...
    def itercounts(self):
        ...
        """
        for condition, sample_dist in self.items():
    ...
    def dump(self, filename):
        ...
        for condition, sample, count in sorted(self.itercounts(),
            ...
            condition = _escape_spaces(str(condition))
            sample = _escape_spaces(str(sample))
    ...
    def to_condition_dist(self):
        ...
        dist = FreqDist()
        for condition, condition_dist in self.items():
            dist.inc(condition, sum(condition_dist.values()))

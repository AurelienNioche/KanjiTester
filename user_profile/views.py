...
def _get_profile_form():
    ...
    syllabus_choices = [(o['tag'], o['tag']) for o in usermodel_models.Syllabus.objects.all().values('tag')]

    class ProfileForm(forms.Form):
        syllabus = forms.ChoiceField(
            required=True, choices=syllabus_choices,
            help_text="The syllabus you would like to study.")
        first_language = forms.CharField(
            max_length=100, required=True,
            help_text="Your first language (native or mother tongue).")
        second_languages = forms.CharField(
